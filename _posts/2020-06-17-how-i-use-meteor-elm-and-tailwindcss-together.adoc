...
We use a Package because it allows us to isolate our Elm application from the rest of the {url-meteor}[Meteor] context.
It is also really useful if we want to remove our Elm application or if one we don't want to use Meteor {url-meteor}[Meteor] anymore.
