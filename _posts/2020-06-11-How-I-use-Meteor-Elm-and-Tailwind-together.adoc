...
Now to run parcel and meteor in //, we will update the `package.json` file.
We will rename the script `start` to `meteor:run`, and redefine the `start` script:
...
The `start` script call `rimraf` to clean the `dist` folder, then we call `concurrently` to run two tasks:
